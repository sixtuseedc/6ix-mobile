...
// needs to change since screens only ever import this component.
...
import type { CheckoutItem } from "../types/models";

interface PaymentModalProps {
  visible: boolean;
  plan: CheckoutItem | null;
  onClose: () => void;
  onSuccess: (plan: CheckoutItem | null) => void;
}

export default function PaymentModal({ visible, plan, onClose, onSuccess }: PaymentModalProps) {
      ...
      <View style={styles.backdrop}>
        <View style={[styles.sheet, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <Text style={[typography.body, { color: colors.textSecondary, marginTop: spacing.xs }]}>
            {plan?.description || (plan?.credits ? `${plan.credits} credits` : "")}
          </Text>

          <View style={styles.priceRow}>
            <Text style={[typography.h2, { color: colors.textPrimary }]}>{plan?.priceLabel}</Text>
          </View>
